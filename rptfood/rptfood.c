/*----------------------------------------------------------------------------
	...
	Author  : Tom Stevelt
	Date    : 2023-2024
	Synopsis: Report of foods in database
...
//xxxxxxxxxxxxx
	...
	{
		case MODE_START:
			StartUp ( "Nutrition Food List", "nutrition", 1, 1, 1, NULL, 0 );

			ptrWebParms = webInitParms ();

			ptrWebParms->WP_FormName   = "rptform";
			ptrWebParms->WP_FormAction = "rptfood.cgi";
		...
		case MODE_RUN:
			ReportUp ( "Food List", "nutrition", NULL, ReportOptions.Format, "food" );
			break;
	}
			...
			if ( getdata () > 0 )
			{
				...
				switch ( ReportOptions.Format )
				{
					...
					case RPT_FORMAT_PDF_VIEW:
						...
						fprintf ( fpBody, "Food List\n" );
						fprintf ( fpBody, "%s\n", "Nutrition" );
						nsFclose ( fpBody );

snprintf ( WhereClause, sizeof(WhereClause), "Mid = %ld", CookieMember.xmid );
LoadMember ( WhereClause, &xmember, 0 );
rptdeliv ( xmember.xmemail, xmember.xmemail, "Food List", fnBody, ReportOptions.OutputFilename, ReportOptions.Format, 0 );
