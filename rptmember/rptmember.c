/*----------------------------------------------------------------------------
	...
	Author  : Tom Stevelt
	Date    : 2023-2024
	Synopsis: Admin only member listing
...
//xxxxxxxxxxxxx
	...
	{
		case MODE_START:
			StartUp ( "Member List", "nutrition", 1, 1, 1, NULL, 0 );

			ptrWebParms = webInitParms ();

			ptrWebParms->WP_FormName   = "rptform";
			ptrWebParms->WP_FormAction = "rptmember.cgi";
		...
		case MODE_RUN:
			ReportUp ( "Member List", "nutrition", NULL, ReportOptions.Format, "member" );
			break;
	}
			...
			if ( getdata () > 0 )
			{
				...
				switch ( ReportOptions.Format )
				{
					...
					case RPT_FORMAT_PDF_VIEW:
						...
						fprintf ( fpBody, "Member List\n" );
						fprintf ( fpBody, "%s\n", "Nutrition" );
						nsFclose ( fpBody );

snprintf ( WhereClause, sizeof(WhereClause), "Mid = %ld", CookieMember.xmid );
LoadMember ( WhereClause, &xmember, 0 );
rptdeliv ( xmember.xmemail, xmember.xmemail, "Member List", fnBody, ReportOptions.OutputFilename, ReportOptions.Format, 0 );
