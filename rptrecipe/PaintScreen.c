...
void PaintScreen ()
{
	...
	if (( Count = dbySelectCount ( &MySql, "recipe", "Rid > 0", LogFileName )) == 0 )
	{
		SaveError ( "No recipes found" );
		return;
	}
	...
	printf ( "<td>\n" );
	printf ( "<select name='ReportStyle'>\n" );
	printf ( "<option value='S'>Recipe Summary</option>\n" );
	printf ( "<option value='D'>Recipe and Food Detail</option>\n" );
	printf ( "</select>\n" );
	printf ( "</td>\n" );
