/*----------------------------------------------------------------------------
	...
	Author  : Tom Stevelt
	Date    : 2023-2024
	Synopsis: Recipe listing, summary or detail.
...
//xxxxxxxxxxxxx
	...
	{
		case MODE_START:
			StartUp ( "Nutrition Recipe List", "nutrition", 1, 1, 1, NULL, 0 );

			ptrWebParms = webInitParms ();

			ptrWebParms->WP_FormName   = "rptform";
			ptrWebParms->WP_FormAction = "rptrecipe.cgi";
		...
		case MODE_RUN:
			ReportUp ( "Recipe List", "nutrition", NULL, ReportOptions.Format, "recipe" );
			break;
	}
	...
	switch ( RunMode )
	{
		...
		case MODE_RUN:
			...
			if (( ReportStyle == 'S' && getdata_smry () > 0 ) ||
				( ReportStyle == 'D' && getdata_dtl  () > 0 ))
			{
				sortdata ();
				...
				switch ( ReportOptions.Format )
				{
					...
					case RPT_FORMAT_PDF_VIEW:
						...
						fprintf ( fpBody, "Recipe List\n" );
						fprintf ( fpBody, "%s\n", "Nutrition" );
						nsFclose ( fpBody );

snprintf ( WhereClause, sizeof(WhereClause), "Mid = %ld", CookieMember.xmid );
LoadMember ( WhereClause, &xmember, 0 );
rptdeliv ( xmember.xmemail, xmember.xmemail, "Recipe List", fnBody, ReportOptions.OutputFilename, ReportOptions.Format, 0 );
