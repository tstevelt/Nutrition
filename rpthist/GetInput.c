...
void GetInput ()
{
	...
	for ( xa = 0; xa < webCount; xa++ )
	{
		...
		else if ( nsStrcmp ( webNames[xa], "Duration" ) == 0 )
		{
			Duration = nsAtoi ( webValues[xa] );
		}
		else if ( nsStrcmp ( webNames[xa], "DayMealFood" ) == 0 )
		{
			DayMealFood = toupper ( webValues[xa][0] );
		}
		else if ( nsStrcmp ( webNames[xa], "EndDate" ) == 0 )
		{
			sprintf ( EndDate, "%s", webValues[xa] );
		}
