...
static int EachHistfood ( XHISTFOOD *ptr )
{
	...
	for ( xa = 0; xa < Count; xa++ )
	{
		if (( nsStrcmp ( Array[xa].Date, xhistory.xhdate  ) == 0 ) &&
			( nsStrcmp ( Array[xa].Meal, xhistory.xhdescr ) == 0 ))
		{
			ndx = xa;
			break;
		}
	...
	if ( xa >= Count )
	{
		...
		sprintf ( Array[Count].Meal, "%s", xhistory.xhdescr );
		switch ( xhistory.xhdescr[0] )
		{
			case 'B': Array[Count].SortField = 1; break;
			case 'L': Array[Count].SortField = 2; break;
			case 'D': Array[Count].SortField = 3; break;
			case 'S': Array[Count].SortField = 4; break;
		}
		ndx = Count;
		Count++;
	}
...
int getdata_meal ()
{
	...
	for ( int xa = 0; xa < Count; xa++ )
	{
		...
		fprintf ( fpData, "%s|%s|%.0f|%.0f|%.0f|%.0f|%.0f|%.2f|%.2f\n",
			Array[xa].Date,
			Array[xa].Meal,
			Array[xa].Calorie,
			Array[xa].Carb,
