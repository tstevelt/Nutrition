...
int Forgot ()
{
	...
	{
		sprintf ( ErrorArray[ErrorCount++].Message, "Unknown email address." );
		rv = -1;
		return ( rv );
	}
	...
	switch ( xmember.xmstatus[0] )
	{
		case 'V':
			...
			snprintf ( Link, sizeof(Link), "http://nutrition.silverhammersoftware.com?forgot=%ld&ipaddr=%s&time=%ld", xmember.xmid, Remote_Address, tvec );
#else
			do
			{
				Salt = rand ();
			...
			snprintf ( KeepThisSecret, sizeof(KeepThisSecret), "%ld:%s:%ld", xmember.xmid, Remote_Address, tvec );
			snprintf ( Link, sizeof(Link), "http://www.nutrition.silverhammersoftware?forgot=%s", tmsEncode(Salt,KeepThisSecret) );
#endif
			...
			SendAttached ( "tstevelt@silverhammersoftware.com",
							...
							"",
							"",
							"Set password on Nutrition",
							1, fn,
							0, NULL );
			
			sprintf ( ErrorArray[ErrorCount++].Message, "New password instructions sent to %s", ForgotEmail );
			break;

		case 'P':
		default:	
			sprintf ( ErrorArray[ErrorCount++].Message, "Account has not been verified!" );
			rv = -4;
			break;
	}
